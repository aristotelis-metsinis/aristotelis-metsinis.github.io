...
function writeDateModified (time, iframe_doc) {
  ...
  // Added by Aristotelis Metsinis. Access iframe's content.
  var date = null;
  if (iframe_doc.contentWindow)
  {
    date = iframe_doc.contentWindow.document.lastModified;
  }
  else if (iframe_doc.contentDocument)
  {
	date = iframe_doc.contentDocument.document.lastModified;
  }
  
  // Assign date variables with document.lastModified 
  var modDate = new Date(Date.parse(date));
  ...
  if (modDate != 0 && modDate != "Invalid Date") {
    ...
    if (time) {
      return (day + " " + ndate + " " + month + " " + year+ "  " + hour + ":" + minute + ":" + second);
    }
  }
	else return ("not available date...");
}